...
describe('decodeOrDefault', () => {
  test('returns default pattern for undefined input', async () => {
    expect(await decodeOrDefault()).toBe('A A A B A A A B\nA A B A A A B A\nA B A A A B A A\nB A A A B A A A\n');
  });

  test('returns default pattern for null', async () => {
    expect(await decodeOrDefault(null)).toBe('A A A B A A A B\nA A B A A A B A\nA B A A A B A A\nB A A A B A A A\n');
  });
  ...
  test('returns empty string for invalid input encoded input', async () => {
    ...
    const encoded = ''
